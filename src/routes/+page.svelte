	...
	async function send() {
		...
		try {
			...
			if (deviceConfig.setAsSleepScreen) {
				await client.setAsSleepScreen(blob);
				sendState = 'success';
				sendMessage =
		...
		<section class="editor-col" aria-label="Editor">
			...
			<div class="panel settings">
				...
				{#if showSettings}
					...
					<p class="hint-small">
						Uploads the card as /sleep.bmp and switches the X3 sleep screen mode to “Custom”, so the
						card shows whenever the device sleeps.
					</p>
